...
def get_configuration(opts: argparse.Namespace) -> Dict:
    ...
    mode = getattr(opts, "model.classification.foodv_new.mode", "tiny")
    if not mode:
        logger.error("Please specify mode")

    mode = mode.lower()
    dropout = getattr(opts, "model.classification.foodv_new.dropout", 0.0)
    norm_layer = getattr(opts, "model.classification.foodv_new.norm_layer", "layer_norm")
    connector_type = getattr(opts, "model.classification.foodv_new.connector_type", "dci")
    print(mode)
